...
| T-M14-09 | — | **Blockiert: das Spiel hat keine Schrift.** `git ls-files` findet null Asset-Dateien, es gibt kein `@font-face` — ausgeliefert wird der Rückfall auf `system-ui`, während `tokens.ts` IBM Plex verlangt und `ASSETS.md` behauptet, die Schriften seien eingebettet. Drei Wächter waren grün, weil sie **eine leere Menge** prüfen. Der Bau verlangt, vier `.woff2` plus OFL-Lizenz (~300 kB) von github.com/IBM/plex herunterzuladen und einzuchecken — ein Download aus externer Quelle, den ich nicht ungefragt vornehme. Beide Wege stehen mit Begründung in PROBLEME.md; Vorschlag: einbetten, weil Richtung A von der schmalen Kartenschrift lebt | ⏳ **wartet auf Noahs Entscheidung** |
